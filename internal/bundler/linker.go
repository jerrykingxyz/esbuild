...
func (c *linkerContext) computeCrossChunkDependencies(chunks []chunkInfo) {
					...
					for partIndex, partMeta := range repr.AST.Parts {
						...
						for _, declared := range part.DeclaredSymbols {
							if declared.IsTopLevel {
								c.graph.Symbols.Get(declared.Ref).ChunkIndexs = append(c.graph.Symbols.Get(declared.Ref).ChunkIndexs, ast.MakeIndex32(uint32(chunkIndex)))
							}
						}
	...
	for chunkIndex := range chunks {
		...
		for importRef := range chunkMeta.imports {
			...
			for _, otherChunkIndex := range c.graph.Symbols.Get(importRef).ChunkIndexs {
				if otherChunkIndex.IsValid() {
					if otherChunkIndex := otherChunkIndex.GetIndex(); otherChunkIndex != uint32(chunkIndex) {
						...
						if otherChunk.entryBits.Contains(chunk.entryBits) {
